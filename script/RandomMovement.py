...
class RandomMovement:
    ...
    def MoveBaseA(self,Loc):
        ...
        rospy.loginfo(f'Moving to {self.Loc.room} at ({self.Goal.target_pose.pose.position.x},{self.Goal.target_pose.pose.position.y}) position')
        self.MBClient.send_goal(self.Goal)

        rospy.loginfo('Waiting for the result ...')
        self.MBClient.wait_for_result()

        dist = sqrt(pow(self.Goal.target_pose.pose.position.x - self.previous_x) +
